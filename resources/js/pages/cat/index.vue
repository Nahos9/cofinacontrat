...
const { data: catData, execute: fetchCAT } = await useApi(
  createUrl("/cat", {
    query: {
      search: searchQuery,
      type: selectedType,
...
const rejectValidationCAT = async (id) => {
  await $api(`cat/reject-validation/${id}`, {
    method: "PUT",
...
const rejectUnblockCAT = async (id) => {
  await $api(`cat/reject-unblock/${id}`, {
    method: "PUT",
    body: { comment: actionComment.value },
  });
  actionComment.value = null;
  ...
  <div>
    ...
    <VCard class="mb-6">
      ...
      >
        ...
        <template #item.status="{ item }">
          <VChip label :color="item.status.color">
            <VTooltip
              ...
              location="start"
              >Raison: {{ item.validation_comment }}</VTooltip
            >
            <VTooltip
        ...
        <template #item.contract.verbal_trial.amount="{ item }">
          {{
            String(item.contract.verbal_trial.amount).replace(
              /\B(?=(\d{3})+(?!\d))/g,
              " "
            )
          }}
              ...
              <VMenu activator="parent">
                <VList>
                  <VListItem
                    ...
                    :to="{
                      name: 'pv-id',
                      params: { id: item.contract.verbal_trial.id },
                    }"
                  >
                    <template #prepend>
                      <VIcon icon="tabler-eye" />
                  ...
                  <VListItem
                    v-if="
                      $can('read', 'contract') || $can('historical', 'contract')
                    "
                    :to="{
                      name: 'contract-id',
                  ...
                  <VListItem
                    ...
                    @click="
                      downloadFile(
                        `/api/cat/download/${item.id}`,
                        `CAT-${item.contract.verbal_trial.committee_id}.docx`
                      )
                    "
                  >
                    <template #prepend>
                  ...
                  <VListItem
                    ...
                    "
                    @click="
                      catSelectedId = item.id;
                      isActionDialogVisible = true;
                      (actionTitle = 'Valider CAT'),
                        (actionText = 'Voulez vous vraiment valider ce CAT?'),
                        (actionFunction = validateCAT);
                  ...
                  <VListItem
                    v-if="
                      $can('reject_validation', 'cat') &&
                      item.validation_status == 'waiting'
                    "
                    @click="
                      catSelectedId = item.id;
                      isActionDialogVisible = true;
                      (actionTitle = 'Rejeter CAT'),
                        (actionText = 'Voulez vous vraiment rejeter ce CAT?'),
                        (actionFunction = rejectValidationCAT);
                  ...
                  <VListItem
                    ...
                    @click="
                      catSelectedId = item.id;
                      isActionDialogVisible = true;
                      (actionTitle = 'Débloquer CAT'),
                        (actionText = 'Voulez vous vraiment débloquer ce CAT?'),
                        (actionFunction = unblockCAT);
                      actionButtonText = 'Débloquer';
                      needComment = false;
                    "
                  >
                    <template #prepend>
                  ...
                  <VListItem
                    ...
                    @click="
                      catSelectedId = item.id;
                      isActionDialogVisible = true;
                      (actionTitle = 'Rejeter deblocage CAT'),
                        (actionText =
                          'Voulez vous vraiment rejeter le déblocage de ce CAT?'),
                        (actionFunction = rejectUnblockCAT);
                      actionButtonText = 'Rejeter';
                      needComment = true;
                    "
                  >
    ...
    <VDialog v-model="isDialogVisible" class="v-dialog-sm">
      ...
      <VCard title="Suppression">
        <VCardText> Etes vous sûr de vouloir supprimer ce CAT? </VCardText>
